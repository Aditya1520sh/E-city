...
const UserLayout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex transition-colors duration-200">
      ...
      <aside className={`fixed h-full z-20 hidden md:block bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 transition-all duration-300 ${isSidebarOpen ? 'w-64 translate-x-0' : 'w-0 -translate-x-full opacity-0 overflow-hidden'}`}>
        <div className="p-6 border-b border-gray-100 dark:border-slate-700">
          <Link to="/" className="flex items-center space-x-3 text-blue-600 dark:text-blue-400">
            <Building2 size={28} />
            <span className="text-xl font-bold text-gray-800 dark:text-white">E-Delhi</span>
          </Link>
        </div>
      ...
      {/* Mobile Header */}
      <div className="md:hidden fixed top-0 w-full bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 z-30 px-4 py-3 flex justify-between items-center">
        <div className="flex items-center space-x-3">
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-gray-500 dark:text-gray-400">
            <Menu size={24} />
          </button>
          <Link to="/" className="flex items-center space-x-2 text-blue-600 dark:text-blue-400">
            <Building2 size={24} />
            <span className="text-lg font-bold text-gray-800 dark:text-white">E-Delhi</span>
          </Link>
        </div>
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-20 bg-black/50" onClick={() => setIsMobileMenuOpen(false)}></div>
      )}

      {/* Mobile Sidebar */}
      <div className={`md:hidden fixed top-0 left-0 h-full w-64 bg-white dark:bg-slate-800 z-40 transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} shadow-xl`}>
        ...
        <nav className="p-4 space-y-2 overflow-y-auto max-h-[calc(100vh-200px)]">
          ...
          <button 
            onClick={handleLogout}
            className="flex items-center space-x-3 px-4 py-3 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 w-full rounded-xl transition-colors mt-4"
          >
            <LogOut size={20} />
            <span>Logout</span>
          </button>
      ...
      {/* Main Content */}
      <main className={`flex-1 p-4 md:p-8 mt-14 md:mt-0 transition-all duration-300 ${isSidebarOpen ? 'md:ml-64' : 'md:ml-0'}`}>
        {children}
      </main>
    </div>
  );
